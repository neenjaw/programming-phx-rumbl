defmodule Rumbl.AccountsTest do
  ...
  describe "authenticate_by_username_and_pass/2" do
    ...
    test "returns user with correct password", %{user: user} do
      assert {:ok, auth_user} = Accounts.authenticate_by_username_and_pw(user.username, @pass)
      assert auth_user.id == user.id
    end

    test "returns unauthorized error with invalid password", %{user: user} do
      assert {:error, :unauthorized} =
               Accounts.authenticate_by_username_and_pw(user.username, "badpass")
    end

    test "returns not found error with no taching user for email" do
      assert {:error, :not_found} = Accounts.authenticate_by_username_and_pw("unknownuser", @pass)
    end
  end
end
